...
async fn test_output_detector_sentence_chunker_no_detections() -> Result<(), anyhow::Error> {
    ...
    let mut chunker_mocks = MockSet::new();
    chunker_mocks.insert(
        MockPath::post(CHUNKER_STREAMING_ENDPOINT),
        Mock::new(
...
#[test(tokio::test)]
async fn test_output_detector_sentence_chunker_with_detections() -> Result<(), anyhow::Error> {
    let detector_name = DETECTOR_NAME_ANGLE_BRACKETS_SENTENCE;
    ...
    detection_mocks.insert(
        ...
        Mock::new(
            ...
            MockResponse::json([vec![ContentAnalysisResponse {
                start: 12,
                end: 15,
                text: "you".into(),
                detection: "has_angle_brackets".into(),
...
/// Asserts that error 503 is propagated from output detector.
#[test(tokio::test)]
async fn test_output_detector_returns_503() -> Result<(), anyhow::Error> {
    let detector_name = DETECTOR_NAME_ANGLE_BRACKETS_SENTENCE;
            ...
            MockResponse::pb_stream(vec![
                ...
                ChunkerTokenizationStreamResult {
                    results: vec![Token {
                        ...
                        text: " Detector, return a 503!".into(),
                    }],
                    token_count: 0,
                    processed_index: 35,
                    start_index: 11,
    ...
    detection_mocks.insert(
        ...
        Mock::new(
            MockRequest::json(ContentAnalysisRequest {
                contents: vec![" Detector, return a 503!".into()],
                detector_params: DetectorParams::new(),
            }),
            MockResponse::json(&expected_detector_error).with_code(StatusCode::SERVICE_UNAVAILABLE),
        ),
    ...
    assert!(second_response.code == 503);
    assert!(
        second_response.details
            == format!(
                "detector request failed for `{}`: {}",
                detector_name, expected_detector_error.message
            )
...
/// Asserts that error 404 is propagated from output detector.
#[test(tokio::test)]
async fn test_output_detector_returns_404() -> Result<(), anyhow::Error> {
    let detector_name = DETECTOR_NAME_ANGLE_BRACKETS_SENTENCE;
            ...
            MockResponse::pb_stream(vec![
                ...
                ChunkerTokenizationStreamResult {
                    results: vec![Token {
                        ...
                        text: " Detector, return a 404!".into(),
                    }],
                    token_count: 0,
                    processed_index: 35,
                    start_index: 11,
    ...
    detection_mocks.insert(
        ...
        Mock::new(
            MockRequest::json(ContentAnalysisRequest {
                contents: vec![" Detector, return a 404!".into()],
                detector_params: DetectorParams::new(),
            }),
            MockResponse::json(&expected_detector_error).with_code(StatusCode::NOT_FOUND),
        ),
    ...
    assert!(second_response.code == 404);
    assert!(
        second_response.details
            == format!(
                "detector request failed for `{}`: {}",
...
/// Asserts that error 500 is propagated from output detector.
#[test(tokio::test)]
async fn test_output_detector_returns_500() -> Result<(), anyhow::Error> {
    let detector_name = DETECTOR_NAME_ANGLE_BRACKETS_SENTENCE;
